...
// Xcode object UUIDs are resolved at runtime (see resolveXcodeUuids). They MUST NOT
// be hard-coded: `expo prebuild --clean` regenerates project.pbxproj with fresh UUIDs,
// and stale literals would make the plugin silently skip embedding the Watch app
// (build passes, but "Apple Watch: Não" in TestFlight).
// expo:targets group UUID is dynamic (generated by @bacons/apple-targets) — found by name at runtime

/** Returns the first real (non-comment) object key for a given isa bucket. */
function firstObjectKey(objects, isa) {
  const bucket = objects[isa] || {};
  return Object.keys(bucket).find(
    (k) => !k.endsWith('_comment') && bucket[k] && typeof bucket[k] === 'object'
  );
}

const unquote = (s) => String(s == null ? '' : s).replace(/^"|"$/g, '');

/**
 * Resolve the Xcode UUIDs this plugin needs by structure/name instead of literals.
 * Throws loudly if any cannot be found — a hard failure beats a silent broken build.
 */
function resolveXcodeUuids(objects) {
  const projectUuid = firstObjectKey(objects, 'PBXProject');
  if (!projectUuid) throw new Error('[with-watch-app] Could not locate PBXProject in project.pbxproj');
  const projectObj = objects['PBXProject'][projectUuid];

  const mainGroupUuid = projectObj.mainGroup;
  if (!mainGroupUuid) throw new Error('[with-watch-app] PBXProject has no mainGroup');

  // Main app target: an application product whose name is neither the Watch app
  // nor a known app-extension target.
  const nativeTargets = objects['PBXNativeTarget'] || {};
  let mainTargetUuid;
  for (const key of Object.keys(nativeTargets)) {
    const t = nativeTargets[key];
    if (key.endsWith('_comment') || !t || typeof t !== 'object') continue;
    const name = unquote(t.name);
    const isApp = String(t.productType || '').includes('com.apple.product-type.application');
    if (isApp && name !== WATCH_TARGET_NAME && name !== 'WorkoutActivity') {
      mainTargetUuid = key;
      break;
    }
  }
  if (!mainTargetUuid) throw new Error('[with-watch-app] Could not find the main app PBXNativeTarget');

  // Products group: the PBXGroup named "Products".
  const groups = objects['PBXGroup'] || {};
  let productsGroupUuid;
  for (const key of Object.keys(groups)) {
    const g = groups[key];
    if (key.endsWith('_comment') || !g || typeof g !== 'object') continue;
    if (unquote(g.name) === 'Products' || unquote(g.path) === 'Products') {
      productsGroupUuid = key;
      break;
    }
  }
  if (!productsGroupUuid) throw new Error('[with-watch-app] Could not find the Products PBXGroup');

  return {
    PROJECT_UUID: projectUuid,
    MAIN_TARGET_UUID: mainTargetUuid,
    MAIN_GROUP_UUID: mainGroupUuid,
    PRODUCTS_GROUP_UUID: productsGroupUuid,
  };
}

function withWatchApp(config) {
  // Read dynamic values from Expo config
  const APPLE_TEAM_ID = config.ios?.appleTeamId || '3D5M22J52M';
  const APP_VERSION = config.version || '1.0.0';
  const BUILD_NUMBER = config.ios?.buildNumber || '1';
  ...
  config = withXcodeProject(config, (config) => {
    ...
    // --- Idempotency check (names may be quoted in the pbxproj) ---
    const nativeTargets = objects['PBXNativeTarget'] || {};
    for (const key of Object.keys(nativeTargets)) {
      if (key.endsWith('_comment')) continue;
      if (typeof nativeTargets[key] === 'object' && unquote(nativeTargets[key].name) === WATCH_TARGET_NAME) {
        console.log(`[with-watch-app] Target "${WATCH_TARGET_NAME}" already exists, skipping.`);
        return config;
      }
    }

    // Resolve UUIDs from the live project (never hard-code — see resolveXcodeUuids).
    const { MAIN_TARGET_UUID, PROJECT_UUID, MAIN_GROUP_UUID, PRODUCTS_GROUP_UUID } =
      resolveXcodeUuids(objects);
    ...
    // --- 13. Add KinevoWatch to Xcode scheme (required for archive/TestFlight) ---
    // Without explicit scheme inclusion, the Watch app builds but is NOT embedded
    // in the archive bundle, causing "Apple Watch: Não" in TestFlight.
    const schemePath = path.join(
      config.modRequest.platformProjectRoot,
      'Kinevo.xcodeproj/xcshareddata/xcschemes/Kinevo.xcscheme'
    );

    if (fs.existsSync(schemePath)) {
      let scheme = fs.readFileSync(schemePath, 'utf8');

      if (!scheme.includes(WATCH_TARGET_NAME)) {
        const watchEntry = `
         <BuildActionEntry
            buildForTesting = "NO"
            buildForRunning = "YES"
            ...
            <BuildableReference
               ...
               ReferencedContainer = "container:Kinevo.xcodeproj">
            </BuildableReference>
         </BuildActionEntry>`;
        ...
        console.log(`[with-watch-app] Added ${WATCH_TARGET_NAME} to Xcode scheme for archive support`);
      }
    }
