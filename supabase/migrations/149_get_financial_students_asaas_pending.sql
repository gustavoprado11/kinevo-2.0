-- ============================================================================
-- Kinevo — 149 get_financial_students: reconhecer status Asaas pending_payment
-- ============================================================================
-- A RPC (migration 141) mapeava só sc.status='pending' → 'awaiting_payment'.
-- O fluxo Asaas usa status='pending_payment' (cobrança criada, aguardando o
-- aluno pagar). Sem tratamento, esses contratos caíam no ELSE → 'courtesy',
-- aparecendo errado na lista/contadores (web e mobile) como cortesia.
--
-- Mudança mínima e aditiva (mesma assinatura de retorno):
--   1. display_status: 'pending_payment' → 'awaiting_payment'
--   2. LATERAL ORDER BY: prioriza 'pending_payment' como 'pending' (peso 2)
--   3. ORDER BY externo: 'pending_payment' junto de 'pending' (peso 5)
-- Nenhuma outra branch alterada.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.get_financial_students(p_trainer_id uuid)
 RETURNS TABLE(student_id uuid, student_name text, avatar_url text, phone text, contract_id uuid, billing_type text, contract_status text, amount numeric, current_period_end timestamp with time zone, block_on_fail boolean, cancel_at_period_end boolean, canceled_by text, canceled_at timestamp with time zone, stripe_subscription_id text, plan_title text, plan_interval text, display_status text, access_blocked_at timestamp with time zone, access_blocked_reason text)
 LANGUAGE plpgsql
