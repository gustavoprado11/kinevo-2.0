-- 082 Distinguish 'expired' from 'canceled' in get_financial_students
--
-- Previously, all canceled contracts returned display_status = 'canceled'.
-- Now we distinguish:
--   - 'expired': contract canceled AND current_period_end has passed (natural expiration)
--   - 'canceled': contract canceled AND current_period_end is in the future (deliberate cancellation)
...
BEGIN
  ...
  ORDER BY
    ...
    CASE
      ...
      -- Expired before canceled in sort order (more actionable)
      WHEN sc.status = 'canceled'
        AND sc.current_period_end IS NOT NULL
        AND sc.current_period_end < now()
        THEN 6
