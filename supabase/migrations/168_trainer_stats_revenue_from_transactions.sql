-- Alinha o card "Receita Mensal" do dashboard mobile do treinador
-- (RPC get_trainer_stats) com a página Financeiro e get_financial_dashboard:
-- soma transações recebidas no mês corrente (Stripe succeeded + Asaas
-- completed) em vez de MRR de student_contracts.status='active'.
--
-- O nome do campo `mrr` na resposta é preservado pra não quebrar consumers
-- existentes (mobile useTrainerDashboard, web get-dashboard-data já passou
-- pela mesma mudança de semântica).
