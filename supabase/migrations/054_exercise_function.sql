-- ============================================================
-- 054: Add exercise_function to workout items
-- Categorizes exercises: warmup, activation, main, accessory, conditioning
-- ============================================================

-- 1. Add column to workout_item_templates
ALTER TABLE workout_item_templates
ADD COLUMN IF NOT EXISTS exercise_function TEXT
    CHECK (exercise_function IS NULL OR exercise_function IN (
        'warmup', 'activation', 'main', 'accessory', 'conditioning'
    ));

-- 2. Add column to assigned_workout_items
...
-- 3. Recreate assign_program_to_student RPC to copy exercise_function
--    (also adds substitute_exercise_ids which was missing since migration 022)
DROP FUNCTION IF EXISTS public.assign_program_to_student(UUID, UUID, TIMESTAMPTZ);
