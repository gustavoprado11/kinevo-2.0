-- ============================================================================
-- Kinevo — 179 get_financial_students: expõe installment_count
-- ============================================================================
-- Adiciona a coluna installment_count ao retorno da RPC, pra web/mobile
-- mostrarem "Parcelado em Nx" no detalhe do aluno. Coluna nova vai no FIM do
-- RETURNS TABLE (aditiva — consumidores que mapeiam por nome não quebram).
--
-- Como o RETURNS TABLE muda, é preciso DROP + CREATE (CREATE OR REPLACE não
-- permite alterar as colunas de saída). Mesma lógica de classificação da 149;
-- só acrescenta sc.installment_count no SELECT.
-- ============================================================================

DROP FUNCTION IF EXISTS public.get_financial_students(uuid);

CREATE FUNCTION public.get_financial_students(p_trainer_id uuid)
...
-- Restaura a postura de segurança da migration 173 (DROP+CREATE reseta grants):
-- só authenticated/service_role executam; anon/public não.
