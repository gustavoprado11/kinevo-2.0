-- ============================================================================
-- 229: expires_at recalculado ao editar programa no editor (M1)
-- ============================================================================
-- Auditoria do builder (docs/analise-builder-2026-07-06.md), achado M1.
--
-- PROBLEMA
-- --------
-- save_assigned_program_tree atualiza duration_weeks/started_at mas nunca
-- expires_at; o cron expire-programs expira pelo valor velho. Cenário:
-- programa ativo de 4 semanas, treinador estende para 8 no editor → o
-- programa expira na semana 4 mesmo assim. Corolário: editar um programa
-- 'expired' re-ativa com expires_at no passado → o cron o re-expira em
-- seguida.
--
-- FIX
-- ---
-- O UPDATE de metadados recalcula expires_at com a MESMA convenção da
-- ativação (computeExpiresAt / RPC 184): started_at + duration_weeks
-- semanas; NULL quando não há started_at (draft/scheduled — a ativação
-- preenche depois) ou duração indefinida (nunca expira).
--
-- Única mudança vs 228: a expressão de expires_at no UPDATE de
-- assigned_programs. Aditivo / backward-compatible.
-- ============================================================================
...
BEGIN
    ...
    UPDATE assigned_programs SET
        ...
        -- FIX (229): estender/encurtar o programa move a expiração junto
        -- (mesma convenção da ativação: started + duration semanas).
        expires_at = CASE
            WHEN (v_prog->>'started_at') IS NOT NULL
                 AND nullif(v_prog->>'duration_weeks','') IS NOT NULL
