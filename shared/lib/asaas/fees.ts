...
// ============================================================================
// FONTE ÚNICA DA VERDADE (web + mobile importam daqui via @kinevo/shared).
// Calibrado via observação real (value vs netValue) em 2026-05.
// Atualizar SÓ aqui quando o Asaas mudar pricing.
//
...
/** Lê o take rate Kinevo do env. Default 0 (zero markup). No mobile não há
 *  esse env, então sempre 0 (a UI mobile só exibe as taxas, não o take rate). */
function getKinevoTakePct(): number {
    const raw =
        (typeof process !== 'undefined' ? process.env?.KINEVO_TAKE_RATE_PCT : undefined) ?? '0'
