...
describe('tool-policy — classificação', () => {
    it('cobre as 55 tools', () => {
        expect(ALL_MCP_TOOLS.length).toBe(55)
        expect(new Set(ALL_MCP_TOOLS).size).toBe(55) // sem duplicatas
    })
...
describe('tool-policy — subsetting', () => {
    it('sem intenção → todas as 55', () => {
        expect(resolveToolSubset([]).length).toBe(55)
    })
    ...
    it('intenção financeiro inclui core + tools financeiras, exclui agenda', () => {
        ...
        expect(subset.length).toBeLessThan(55) // cortou o input
    })
