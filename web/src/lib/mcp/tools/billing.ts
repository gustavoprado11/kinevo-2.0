...
export function registerBillingReadTools(server: McpServer, trainerId: string) {
  server.tool(
    ...
    { title: 'Listar assinaturas', readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ status, limit }) => {
  ...
  server.tool(
    ...
    { title: 'Resumo de receita', readOnlyHint: true, destructiveHint: false, openWorldHint: false },
    async ({ month }) => {
      const supabaseAdmin = createAdminClient()
