        ...
        >
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="mt-1 text-muted-foreground">Gerencie sua biblioteca e acesse exercícios do sistema</p>
                </div>

                <button
                    onClick={handleCreate}
                    className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-violet-600 to-blue-600 px-4 py-2.5 text-sm font-medium text-white shadow-lg shadow-violet-500/20 transition-all hover:from-violet-500 hover:to-blue-500"
                >
            ...
            {/* List Container */}
            <div className="min-h-[500px] rounded-xl border border-border bg-card p-6 shadow-sm">
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border border-input bg-card py-2.5 pl-10 pr-4 text-foreground placeholder:text-muted-foreground transition-all focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-primary/30"
                            />
                        </div>
                        <button
                            onClick={() => setIsManagerOpen(true)}
                            className="flex items-center gap-2 whitespace-nowrap rounded-lg border border-border bg-secondary px-4 py-2.5 text-sm font-medium text-secondary-foreground transition-all hover:bg-secondary/80"
                        >
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition-all ${isSelected
                                            ...
                                            : 'bg-card text-muted-foreground border-border hover:border-border/80 hover:text-foreground/80'
                                            }`}
                                    >
                                        {group}
                                    </button>
                    ...
                    {filteredExercises.length > 0 ? (
                        ...
                        <div className="text-center py-16 bg-muted/40 rounded-xl border border-border border-dashed">
                            ...
                            <p className="text-muted-foreground text-sm mt-1">Tente buscar por outro termo ou limpar os filtros</p>
                            <button
                                onClick={() => { setSearchQuery(''); setSelectedMuscleGroups([]) }}
                                className="mt-4 rounded-lg border border-border bg-secondary px-4 py-2 text-sm text-secondary-foreground transition-colors hover:bg-secondary/80"
                            >
                                Limpar busca
                            </button>
                        </div>
