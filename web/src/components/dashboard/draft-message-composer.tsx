...
interface DraftMessageComposerProps {
    ...
    studentName: string
    onClose: () => void
    /** Chamado após envio bem-sucedido — o pai remove o insight do feed. */
    onSent: (insightId: string) => void
}
...
export function DraftMessageComposer({
    insight, studentId, studentName, onClose, onSent,
    ...
    // Geração LAZY: dispara o rascunho só ao montar (no clique em "Mensagem").
    useEffect(() => {
        let cancelled = false
        ;(async () => {
            try {
                const res = await fetch('/api/assistant/draft-message', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ insight_id: insight.id, student_id: studentId }),
                })
                if (!res.ok) {
                    const txt = await res.text().catch(() => '')
                    if (!cancelled) {
                        setError(txt || 'Não foi possível gerar o rascunho.')
                        setPhase('error')
                    }
                    return
                }
                ...
                if (!cancelled) {
                    setError('Erro de rede ao gerar o rascunho.')
                    setPhase('error')
                }
            }
        })()
        return () => { cancelled = true }
    }, [insight.id, studentId])
    ...
    const handleSend = useCallback(async () => {
        ...
        if (res.success) {
            // Não bloqueia o fechamento se o mark falhar; o envio já ocorreu.
            await markInsightActed(insight.id)
            onSent(insight.id)
        } else {
            setError(res.error || 'Falha ao enviar a mensagem.')
            setSending(false)
        }
    }, [message, sending, studentId, insight.id, onSent])
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4">
                    {phase === 'loading' && (
                        <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
                            ...
                            <p className="text-[13px] text-[#6E6E73] dark:text-k-text-secondary">
                                Gerando rascunho com base nos dados do aluno…
                            </p>
                        </div>
                    )}
                    ...
                    {phase === 'error' && (
                        <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
                            ...
                            <p className="text-[13px] text-[#6E6E73] dark:text-k-text-secondary">
                                {error || 'Não foi possível gerar o rascunho.'}
                            </p>
                        </div>
                    )}
                    ...
                    {phase === 'ready' && (
                        <>
                            {confidence === 'low' && (
                                <div className="mb-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-500/20 dark:bg-amber-500/10">
                                    ...
                                    <p className="text-[11px] text-amber-700 dark:text-amber-400">
                                        Contexto limitado deste aluno — revise com atenção antes de enviar.
                                    </p>
                                </div>
                            )}
                            ...
                            {references.length > 0 && (
                                <div className="mt-3">
                                    <p className="mb-1 text-[10px] font-medium uppercase tracking-wide text-[#AEAEB2] dark:text-k-text-quaternary">
                                        Ancorado em
                                    </p>
                                    <div className="flex flex-wrap gap-1.5">
                                        {references.map((r, i) => (
                                            <span
                                                key={i}
                                                className="rounded-full bg-[#F5F5F7] px-2 py-0.5 text-[10px] text-[#6E6E73] dark:bg-glass-bg dark:text-k-text-secondary"
                                            >
                                                {r}
                                            </span>
                                        ))}
                ...
                <div className="flex items-center justify-end gap-2 border-t border-[#E8E8ED] px-5 py-3 dark:border-k-border-subtle">
                    <button
                        onClick={onClose}
                        disabled={sending}
                        className="rounded-lg px-3 py-1.5 text-[13px] font-medium text-[#6E6E73] transition-colors hover:bg-[#F5F5F7] disabled:opacity-50 dark:text-k-text-secondary dark:hover:bg-muted"
                    >
                        Descartar
                    </button>
                    <button
                        onClick={handleSend}
                        disabled={phase !== 'ready' || sending || !message.trim()}
                        className="flex items-center gap-1.5 rounded-lg bg-violet-600 px-3.5 py-1.5 text-[13px] font-medium text-white transition-colors hover:bg-violet-500 disabled:opacity-50"
                    >
