    ...
    return (
        <div className="bg-gray-800/50 rounded-xl border border-gray-700/50 p-6">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h2 className="text-lg font-semibold text-white">Resumo de Treinos</h2>
                    <p className="text-sm text-gray-400 mt-0.5">Histórico de atividades</p>
                </div>
            </div>

            {!hasHistory ? (
                <div className="text-center py-6">
                    <div className="w-14 h-14 rounded-full bg-gray-700/50 flex items-center justify-center mx-auto mb-3">
                        <svg className="w-7 h-7 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                        </svg>
                    </div>
                    <p className="text-gray-400 text-sm">Nenhum treino realizado ainda</p>
                </div>
            ) : (
                <div className="space-y-6">
                    <div className="grid grid-cols-3 gap-4">
                        {/* Total Sessions */}
                        <div className="bg-gray-900/50 rounded-xl border border-gray-700/30 p-4 text-center">
                            ...
                            <p className="text-2xl font-bold text-white mb-1">{summary.totalSessions}</p>
                            <p className="text-xs text-gray-400">Treinos totais</p>
                        </div>
                        ...
                        <div className="bg-gray-900/50 rounded-xl border border-gray-700/30 p-4 text-center">
                            ...
                            <p className="text-2xl font-bold text-white mb-1">{summary.completedThisWeek}</p>
                    ...
                    {recentSessions.length > 0 && (
                        <div>
                            <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-3">Últimas Sessões</h3>
                            <div className="space-y-2">
                                {recentSessions.map((session) => (
                                    <button
                                        key={session.id}
                                        onClick={() => handleSessionClick(session.id)}
                                        className="w-full bg-gray-900/30 hover:bg-gray-800/50 border border-gray-700/30 hover:border-violet-500/30 rounded-lg p-3 flex items-center justify-between transition-all group"
                                    >
