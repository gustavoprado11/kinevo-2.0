...
export function SessionDetailSheet({ isOpen, onClose, sessionId }: SessionDetailSheetProps) {
    ...
    const getRpeColor = (rpe: number | null) => {
        if (!rpe) return 'bg-muted/50 text-muted-foreground'
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Sheet Panel */}
            <div className="relative w-full max-w-md h-full bg-background border-l border-border shadow-2xl flex flex-col transform transition-transform duration-300 ease-out">
                {/* Header */}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    {isLoading ? (
                        ...
                        <>
                            {/* Feedback Section */}
                            <div className="bg-muted/40 rounded-xl p-5 border border-border/70">
                                <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider mb-4">Feedback do Aluno</h3>
                                <div className="flex gap-4">
                                    ...
                                    <div className="flex-1">
                                        {details.feedback ? (
                                            <div className="bg-card rounded-lg p-3 h-full border border-border/70 italic text-foreground/80 text-sm relative">
                                                <svg className="w-4 h-4 text-muted-foreground/80 absolute top-2 left-2 opacity-50" fill="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div className="h-full flex items-center text-muted-foreground text-sm italic pl-2">
                            ...
                            <div className="space-y-6">
                                <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Exercícios Realizados</h3>

                                {details.exercises.map((exercise) => (
                                    <div key={exercise.exercise_id} className="bg-muted/30 rounded-xl overflow-hidden border border-border/70">
                                        <div className="px-4 py-3 bg-muted/50 border-b border-border/70">
                                            <h4 className="font-medium text-foreground">{exercise.name}</h4>
                                            {exercise.muscle_group && (
                                                <span className="text-xs text-muted-foreground capitalize">{exercise.muscle_group}</span>
                                            )}
                                        </div>
                                                ...
                                                <tbody className="divide-y divide-gray-700/30">
                                                    {exercise.sets.map((set, idx) => (
                                                        <tr key={idx} className="text-foreground/80">
                                                            <td className="py-2 pl-2 text-muted-foreground font-mono text-xs">{set.set_number}</td>
