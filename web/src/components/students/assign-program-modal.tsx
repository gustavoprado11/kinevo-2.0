...
export function AssignProgramModal({
    ...
    // Track mounted state.
    // React 18 strict mode (e o ciclo do `next/dynamic({ ssr: false })`) monta →
    // desmonta → remonta. Sem o reset explícito ao montar, o cleanup da primeira
    // montagem deixava `isMounted.current === false` pra sempre, e o
    // `setLoading(false)` no finally do fetch ficava gated → loading eterno.
