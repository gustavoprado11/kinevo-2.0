...
**~~⚠️ BLOQUEADOR EXTERNO~~ → RESOLVIDO 08/jul:** a criação da push subscription tinha falhado com `403 Application Status: Inactive` — o app do Developer Program Standard Tier exige assinatura Strava paga na conta dona. **Gustavo assinou o Strava e reativou o app 244932** (painel /settings/api voltou a carregar). Com o app ativo, `strava-webhook-setup` **criou a push subscription (id 360547)** na hora, idempotência confirmada (2ª chamada `action:none`). Webhook Strava no ar.

**Última milha (Strava):** o único aluno com Strava segue `status=error` ("abra o app Kinevo") até abrir o app 1×. Detalhe bom: **não precisa do build novo** — a persistência do token vive na edge `strava-token-refresh` v4 (já em prod), então o app ATUAL, ao refrescar o token Strava expirado, já grava o token server-side com `external_user_id`. A partir daí o webhook mapeia+ingere em tempo real e o reconcile sincroniza. Até lá o webhook responde 200 `unmapped_athlete` (sem ingerir) e o watchdog mantém o status honesto.
...
Tudo isso é mobile/DB; o realtime (migration 237) está em prod. As Fases 1+2 mobile entram juntas no build EAS 1.5.7. **Frente Saúde: Oura+Strava+watchdog+realtime em prod; só resta cortar o build EAS 1.5.7 (leva F1+F2 mobile) e o aluno abrir o app 1× p/ reativar o Strava dele.**
