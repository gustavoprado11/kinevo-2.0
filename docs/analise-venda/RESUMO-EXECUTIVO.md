...
| 12 | **Dunning com período de graça** | ✅ | — | **FEITO + deployado + E2E live**: `past_due` mantém o tier pago por **14 dias** (`current_period_end + 14d`), com fail-safe time-bounded (lição do webhook-miss de abril). E2E via `ai-status`: 3d→`pro_ia` (300 cr, alunos livres), 30d→`free`. Commit `7760083` | — |
